...
func BuildIndex(ctx context.Context, projectPath string, cfg Config, vdb *VectorDB, invIndex *InvertedIndex) (*IndexStats, error) {
	// 1. 扫描文件
	...
	for _, doc := range docs {
		...
		chunks := SplitByStructure(doc.Content, lang, doc.FilePath, cfg.MaxChunkSize)
		...
		for _, chunk := range chunks {
			vector, err := GetEmbedding(cfg, chunk.Content)
			if err != nil {
