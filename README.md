...
cp config-examples/.env.gemini.example .env         # Google Gemini API
...
| `config-examples/.env.gemini.example`            | Google Cloud | 使用 Google Gemini API                   |
...
| 场景              | 说明                                                                   |
| ----------------- | ---------------------------------------------------------------------- |
| **单个项目**      | 将 `start-mcp.sh` 和 `code-context-mcp` 放在同一目录（如项目根目录）   |
...
```bash
# 1. 将 start-mcp.sh 和 code-context-mcp 放在同一目录
...
> 提示：如果你是从源码构建，可以使用 `make deploy` 自动将二进制文件和脚本部署到当前目录：
>
> ```bash
> make deploy
> ```
>
> 或者部署到指定目录：`make deploy DEPLOY_DIR=/path/to/target`
...
# 生成 start-mcp.sh 脚本（从模板生成，包含版本信息）
make gen-script

# 构建二进制文件并生成脚本，然后部署到当前目录
make deploy

# 部署到指定目录
make deploy DEPLOY_DIR=/path/to/target
