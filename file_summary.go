...
func ExtractSummary(content string, lang string, filePath string) *FileSummary {
	...
	case "md":
		extractMDSummary(content, summary)
	case "py":
		extractPySummary(content, summary)
	default:
		// 仅返回基本信息
	}
...
func extractGoSummary(content string, summary *FileSummary) {
	lines := strings.Split(content, "\n")
	inImportBlock := false
...
func extractVueSummary(content string, summary *FileSummary) {
	// 提取 script 部分的摘要
...
func extractJSTSSummary(content string, summary *FileSummary, lang string) {
	lines := strings.Split(content, "\n")
...
func extractMDSummary(content string, summary *FileSummary) {
	lines := strings.Split(content, "\n")
	for i, line := range lines {
...
func extractPySummary(content string, summary *FileSummary) {
	lines := strings.Split(content, "\n")
