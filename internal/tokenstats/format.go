...
func FormatStats(stats StatsSnapshot) string {
	...
	if len(stats.ByTool) > 0 {
		b.WriteString("\n按工具分组:\n")
		b.WriteString("工具              调用次数  输出token   节省token   平均耗时\n")
		...
		for _, name := range names {
			...
			fmt.Fprintf(&b, "%-16s  %8d  %10s  %10s  %8dms\n",
				name,
				tool.CallCount,
	...
	if len(stats.Daily) > 0 {
		...
		for _, d := range stats.Daily[start:] {
			...
			fmt.Fprintf(&b, "%s    %8d  %10s\n", shortDate, d.CallCount, formatNumber(d.SavedTokens))
		}
	}
